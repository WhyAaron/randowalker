...
function App() {
  ...
  >("areaSelection");
  const [startingPosition, setStartingPosition] = useState<LatLngTuple | null>(
    null
  );
  ...
  const { graph, buildGraph, findConnectedMarkers, findPathBFS } = useGraph();
  ...
  function generateRandomPath(): number[][] {
    const mainPathPoints: number[][] = generateMainPathPoints();
    ...
    <div className="w-full h-full grid grid-cols-5">
      ...
      <div className="w-full h-full col-span-4 z-0">
        ...
        >
          ...
          />
          {!graph && <Segments segments={segments} points={nodes} />}
          {!graph && (
            <Nodes
              segments={segments}
              nodes={nodes}
